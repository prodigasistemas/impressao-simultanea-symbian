...
public class BackgroundTaskReceberArquivoIdaOnline extends BackgroundTask {
	...
	try {
	    ...
	    InputStream is = MessageDispatcher.getInstancia().getRespostaOnline();
	    boolean primeiraVez = true;
	    
	    if ( MessageDispatcher.getInstancia().getTipoArquivo() == Constantes.TIPO_ARQUIVO_GZ ){
		is = Util.inflateFile(is, MessageDispatcher.getInstancia().getFileLength() );
	    }

	    do {
		linhas = FileManager.getInstancia().lerArquivoImoveis(is);
		if (primeiraVez && (linhas == null || linhas.size() == 0)) {
		    error = true;
		    break;
		} else {
		    primeiraVez = false;
	    ...
	    } while ((linhas.size() == 65) || (linhas.lastElement() == null));
		...
		Repositorio.indicesCarregados = true;
	    	...
	    	Util.mostrarErro("Nenhum arquivo encontrado no servidor...");
	...
	if (!error) {
	    ...
	    FachadaRede.getInstancia().confirmarRecebimentoArquivo();
	    TelaLogin.getInstancia().criarTelaLogin();
	} else {
	    // Caso qualquer error aconteça no carregamento do arquivo
